...
const PD = {
  ...
  // ╚═══════════════════════════════════════════════════════════╝
  bahias:           {mean:8,std:1,min:3,max:18,label:"Puestos de trabajo",unit:"u",group:"cap",lever:false},
  dias_op:          {mean:24,std:1,min:20,max:26,label:"Días operativos / mes",unit:"d",group:"cap",lever:false},
  util_pct:         {mean:70,std:10,min:35,max:95,label:"Utilización puestos %",unit:"%",group:"cap",lever:true},
  hrs_std_ot:       {mean:1.2,std:0.3,min:0.5,max:3,label:"Hrs estándar por OT",unit:"h",group:"cap",lever:false},
  tiempo_muerto:    {mean:15,std:5,min:5,max:35,label:"Tiempo muerto puesto %",unit:"%",group:"cap",lever:true},
  asesores:         {mean:2,std:0.5,min:1,max:4,label:"Asesores / recepcionistas",unit:"u",group:"cap",lever:false},
  ot_por_asesor:    {mean:12,std:2,min:6,max:22,label:"OTs por asesor por día",unit:"u",group:"cap",lever:true},
  no_show:          {mean:8,std:3,min:2,max:25,label:"No-show / cancelación %",unit:"%",group:"cap",lever:true},

  // ╔═══════════════════════════════════════════════════════════╗
  // ║  PRODUCTIVIDAD TÉCNICOS                                  ║
  // ╚═══════════════════════════════════════════════════════════╝
  tecnicos:         {mean:6,std:1,min:2,max:15,label:"Técnicos (planilla)",unit:"u",group:"tech",lever:false},
  hrs_disp:         {mean:8,std:0.3,min:7,max:9,label:"Hrs disponibles / técnico / día",unit:"h",group:"tech",lever:false},
  eficiencia:       {mean:80,std:8,min:55,max:110,label:"Eficiencia % (hrs fact / disp)",unit:"%",group:"tech",lever:true},
  productividad:    {mean:92,std:5,min:70,max:105,label:"Productividad % (rapidez vs estándar)",unit:"%",group:"tech",lever:true},
  tarifa_hr:        {mean:15,std:3,min:8,max:30,label:"Tarifa hora MO",unit:"$",group:"tech",lever:true},

  // ╔═══════════════════════════════════════════════════════════╗
  // ║  REPUESTOS Y VENTA ADICIONAL                             ║
  // ╚═══════════════════════════════════════════════════════════╝
  ratio_rep:        {mean:0.70,std:0.12,min:0.3,max:1.3,label:"Ratio repuestos / MO ($)",unit:"x",group:"margen",lever:true},
  margen_mo:        {mean:60,std:6,min:40,max:80,label:"Margen mano de obra %",unit:"%",group:"margen",lever:false},
  margen_rep:       {mean:30,std:5,min:15,max:50,label:"Margen repuestos %",unit:"%",group:"margen",lever:false},
  venta_adic_pct:   {mean:30,std:8,min:5,max:60,label:"% OTs con venta accesorios/adic",unit:"%",group:"margen",lever:true},
  ticket_adic:      {mean:35,std:10,min:10,max:100,label:"Ticket promedio accesorios/adic",unit:"$",group:"margen",lever:true},

  // ╔═══════════════════════════════════════════════════════════╗
  // ║  RETENCIÓN Y DEMANDA                                     ║
  // ╚═══════════════════════════════════════════════════════════╝
  retencion:        {mean:35,std:10,min:15,max:65,label:"Retención clientes %",unit:"%",group:"demanda",lever:true},
  clientes_ext:     {mean:150,std:40,min:40,max:400,label:"Clientes externos nuevos / mes",unit:"u",group:"demanda",lever:true},
  clientes_parque:  {mean:120,std:25,min:30,max:350,label:"Clientes del parque (motos vendidas) / mes",unit:"u",group:"demanda",lever:true},

  // ╔═══════════════════════════════════════════════════════════╗
  // ║  COSTOS DE PERSONAL Y FIJOS                              ║
  // ╚═══════════════════════════════════════════════════════════╝
  sueldo_tec:       {mean:500,std:80,min:300,max:1000,label:"Sueldo técnico / mes",unit:"$",group:"costos",lever:false},
  sueldo_asesor:    {mean:600,std:100,min:350,max:1200,label:"Sueldo asesor / mes",unit:"$",group:"costos",lever:false},
  jefe_taller:      {mean:1200,std:200,min:700,max:2500,label:"Sueldo jefe taller / mes",unit:"$",group:"costos",lever:false},
  gastos_fijos:     {mean:3000,std:600,min:1000,max:8000,label:"Otros gastos fijos / mes",unit:"$",group:"costos",lever:false},

  // ╔═══════════════════════════════════════════════════════════╗
  // ║  OVERHEAD ASIGNADO Y D&A                                 ║
  // ╚═══════════════════════════════════════════════════════════╝
  overhead_asignado:{mean:8000,std:1500,min:3000,max:20000,label:"Overhead asignado al taller / mes",unit:"$",group:"overhead",lever:false},
  deprec_equipos:   {mean:1000,std:200,min:200,max:3000,label:"Depreciación equipos taller / mes",unit:"$",group:"overhead",lever:false},
  amort_software:   {mean:300,std:100,min:0,max:1000,label:"Amortización DMS/software / mes",unit:"$",group:"overhead",lever:false},

  // ╔═══════════════════════════════════════════════════════════╗
  // ║  EVA                                                     ║
  // ╚═══════════════════════════════════════════════════════════╝
  tasa_imp:         {mean:30,std:0,min:30,max:30,label:"Tasa impositiva % (IR)",unit:"%",group:"eva_p",lever:false},
  capital_taller:   {mean:150000,std:20000,min:50000,max:400000,label:"Capital invertido taller",unit:"$",group:"eva_p",lever:false},
...
// ═══ MAIN ═══
export default function TallerMotosMonteCarlo(){
  ...
  const GC={
    cap:{t:"Puestos, Asesores y Tiempos",c:C.blue,i:"🔧"},
  ...
  return(
    <div style={{"--serif":"'Cormorant Garamond',serif","--sans":"'Outfit',sans-serif","--mono":"'JetBrains Mono',monospace",
      ...
      <div style={{background:`linear-gradient(135deg,${C.deep} 0%,${C.green} 100%)`,padding:"14px 12px 10px",color:"#fff"}}>
        ...
        <div style={{fontFamily:"var(--serif)",fontSize:15,fontWeight:700}}>Simulador Monte Carlo — Taller Motos</div>
        <div style={{fontSize:7,opacity:.7,letterSpacing:1.5,textTransform:"uppercase"}}>Taller de Motocicletas · Modelo Causal + Goal-Seeking</div>
      </div>
      ...
      <div style={{padding:"8px 8px 36px"}}>
        ...
        {tab==="goalseeking"&&(
          <div>
            <div style={{background:C.card,borderRadius:6,padding:10,border:`1px solid ${C.border}`,marginBottom:8,borderTop:`3px solid ${C.gold}`}}>
              <div style={{fontFamily:"var(--serif)",fontSize:13,fontWeight:700,color:C.deep,marginBottom:6}}>🎯 Meta del Taller de Motos</div>
          ...
          <div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:5,marginBottom:6}}>
              ...
              <div style={{background:C.card,borderRadius:5,padding:"7px 9px",border:`1px solid ${C.border}`}}>
                <div style={{fontSize:7,color:C.muted}}>ROT. PUESTO/DÍA</div>
            ...
            <div style={{background:C.card,borderRadius:6,padding:10,border:`1px solid ${C.border}`,marginBottom:6}}>
              <div style={{fontFamily:"var(--serif)",fontSize:12,fontWeight:700,color:C.deep,marginBottom:4}}>P&L Taller Motos — Mediana Anual</div>
              {[
        ...
        <div style={{marginTop:12,textAlign:"center",fontSize:7,color:C.muted}}>© Promundial Consulting Group · Monte Carlo Taller Motos · Nicaragua IR 30%</div>
      </div>
    </div>
  );
}
